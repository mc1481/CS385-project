...
function VeganSearch() {
  ...
  // function to get vegan recipes
  async function fetchRecipes() {
    setLoading(true);
    try {
        const response = await axios.get(
          `https://api.spoonacular.com/recipes/complexSearch?apiKey=${apiKey}&query=${searchTerm}&number=20&diet=vegan`
        );
  ...
  return (
    <div>
      ...
      {noResults ? (
        <p>Sorry, there are no vegan recipes matching your search</p>
      ) : null}
